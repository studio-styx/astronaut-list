...
export default async function endAnalise(interaction: ChatInputCommandInteraction<"cached">) {
        ...
        fields: [
            {
                ...
                value: annotations.length !== 0 ? annotations.map((a, i) => `${i + 1}. | \`${a.text}\``).join("\n") : "Nenhum erro encontrado"
            },
            {
                name: "analise",
                value: justification
    ...
    if (approved) {
        ...
        const botUser = await interaction.guild.members.fetch(user.analising);
        const botOwner = await interaction.guild.members.fetch(application.userId);

        try {
            await botUser.roles.add(settings.guild.roleBotAprroved);
            await botOwner.roles.add(settings.guild.devRole)
        } catch (e: any) {
            console.log(e);
        ...
        try {
            await interaction.guild.members.kick(user.analising);
        } catch (e) {
            console.log(e);
            await interaction.guild.channels.fetch(channelId).then(async channel => {
                if (channel?.isTextBased()) {
