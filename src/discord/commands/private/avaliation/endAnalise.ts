...
export default async function endAnalise(interaction: ChatInputCommandInteraction<"cached">) {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (approved) {
            ...
            const components = [
                `${!ownerUser ? "Não encontrado" : userMention(ownerUser.id)} - **${icon.block} | Seu bot foi reprovado!**`,
                createSeparator(),
                createSection({
                    content: brBuilder(
                        `## ( ${icon.bug} ╺╸ Erros encontrados durante a analise:`,
    ...
    try {
        ...
        const thread: { [key: string]: { replicated: string } } = JSON.parse(fs.readFileSync(rootPath, "utf8"));
